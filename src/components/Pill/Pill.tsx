...
export const Pill: FunctionComponent<PillProps> = ({
  ...
  return type === 'transparent' ? (
    <div
      key={label}
      className="group h-[40px] text-left ml-[10px] md:!ml-2 lg:!ml-3 px-[15px] py-[0.5rem] first:ml-0 bg-white border border-black rounded-[15px] text-black text-[15px] font-normal font-['FuturaBT-Medium'] whitespace-nowrap cursor-pointer"
      onClick={onClick}
    >
      <span className="ds-sdk-pill__label font-normal opacity-50">{label}</span>
      <span className="ds-sdk-pill__cta">
        {CTA}
      </span>
    </div>
  ) : (
